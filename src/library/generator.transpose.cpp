...
namespace clfft_transpose_generator {
    ...
    void OffsetCalc(fmt::memory_buffer &transKernel, const FFTKernelGenKeyParams &params, bool input) {
        const size_t *stride = input ? params.fft_inStride : params.fft_outStride;
        std::string_view offset = input ? "iOffset" : "oOffset";

        clKernWrite(transKernel, 3, "size_t {} = 0;\n", offset);
        clKernWrite(transKernel, 3, "g_index = get_group_id(0);\n");

        for (size_t i = params.fft_DataDim - 2; i > 0; i--) {
            clKernWrite(transKernel, 3, "{} += (g_index / numGroupsY_{}) * {};\n", offset, i, stride[i + 1]);
            clKernWrite(transKernel, 3, "g_index = g_index % numGroupsY_{};\n", i);
        }
...
// the generator that it wants the twiddle factors generated inside of the transpose
    ...
    genTwiddleMath(const FFTKernelGenKeyParams &params, fmt::memory_buffer &transKernel, std::string_view dtComplex,
        ...
        clKernWrite(transKernel, 9, "{} Wm = TW3step( (t_gx_p*32 + lidx) * (t_gy_p*32 + lidy + loop*8) );\n",
                    dtComplex);
        clKernWrite(transKernel, 9, "{} Wt = TW3step( (t_gy_p*32 + lidx) * (t_gx_p*32 + lidy + loop*8) );\n",
                    dtComplex);
        clKernWrite(transKernel, 9, "{} Tm, Tt;\n", dtComplex);

        if (fwd) {
            clKernWrite(transKernel, 9, "Tm.x = ( Wm.x * tmpm.x ) - ( Wm.y * tmpm.y );\n");
            clKernWrite(transKernel, 9, "Tm.y = ( Wm.y * tmpm.x ) + ( Wm.x * tmpm.y );\n");
            clKernWrite(transKernel, 9, "Tt.x = ( Wt.x * tmpt.x ) - ( Wt.y * tmpt.y );\n");
            clKernWrite(transKernel, 9, "Tt.y = ( Wt.y * tmpt.x ) + ( Wt.x * tmpt.y );\n");
        } else {
            clKernWrite(transKernel, 9, "Tm.x =  ( Wm.x * tmpm.x ) + ( Wm.y * tmpm.y );\n");
            clKernWrite(transKernel, 9, "Tm.y = -( Wm.y * tmpm.x ) + ( Wm.x * tmpm.y );\n");
            clKernWrite(transKernel, 9, "Tt.x =  ( Wt.x * tmpt.x ) + ( Wt.y * tmpt.y );\n");
        ...
        clKernWrite(transKernel, 9, "tmpm.x = Tm.x;\n");
        clKernWrite(transKernel, 9, "tmpm.y = Tm.y;\n");
        clKernWrite(transKernel, 9, "tmpt.x = Tt.x;\n");
        clKernWrite(transKernel, 9, "tmpt.y = Tt.y;\n");
...
// the generator that it wants the twiddle factors generated inside of the transpose
    ...
    genTwiddleMathLeadingDimensionBatched(const FFTKernelGenKeyParams &params, fmt::memory_buffer &transKernel,
        ...
        if (params.fft_N[0] > params.fft_N[1]) {
            clKernWrite(transKernel, 9,
                        "{} Wm = TW3step( ({} * square_matrix_index + t_gx_p*32 + lidx) * (t_gy_p*32 + lidy + loop*8) );\n",
                        dtComplex, params.fft_N[1]);
            clKernWrite(transKernel, 9,
                        "{} Wt = TW3step( ({} * square_matrix_index + t_gy_p*32 + lidx) * (t_gx_p*32 + lidy + loop*8) );\n",
                        dtComplex, params.fft_N[1]);
        } else {
            clKernWrite(transKernel, 9,
                        "{} Wm = TW3step( (t_gx_p*32 + lidx) * ({} * square_matrix_index + t_gy_p*32 + lidy + loop*8) );\n",
                        dtComplex, params.fft_N[0]);
            clKernWrite(transKernel, 9,
                        "{} Wt = TW3step( (t_gy_p*32 + lidx) * ({} * square_matrix_index + t_gx_p*32 + lidy + loop*8) );\n",
                        dtComplex, params.fft_N[0]);
        }
        clKernWrite(transKernel, 9, "{} Tm, Tt;\n", dtComplex);
        ...
        if (params.fft_placeness == CLFFT_OUTOFPLACE)
            switch (params.fft_outputLayout) {
                case CLFFT_COMPLEX_INTERLEAVED:
                    dtInput = dtComplex;
                    dtOutput = dtComplex;
                    clKernWrite(transKernel, 0, ", global {}* restrict outputA", dtOutput);
                    break;
                case CLFFT_COMPLEX_PLANAR:
                    dtInput = dtPlanar;
                    dtOutput = dtPlanar;
                    clKernWrite(transKernel, 0, ", global {}* restrict outputA_R, global {}* restrict outputA_I",
                                dtOutput, dtOutput);
                    break;
                case CLFFT_HERMITIAN_INTERLEAVED:
                case CLFFT_HERMITIAN_PLANAR:
...
//this function assumes a 1:2 ratio
    clfftStatus genSwapKernel(const FFTGeneratedTransposeNonSquareAction::Signature &params, std::string &strKernel,
        ...
        /*Generating the  swapping logic*/
        {
            ...
            for (size_t i = 0; i < cycle_map[0]; i++) {
                ...
                clKernWrite(transKernel, 0) << "{  " << start_inx << ",  " << cycle_map[inx + 1] << ",  0},"
                                            << std::endl;
                ...
                while (start_inx != cycle_map[++inx]) {
                    ...
                    clKernWrite(transKernel, 0) << "{  " << cycle_map[inx] << ",  " << cycle_map[inx + 1] << ",  "
                                                << action_var << "}," << std::endl;
                    if (action_var == 2)
                        cycle_stat[stat_idx++] = num_swaps;
                    num_swaps++;
            ...
            clKernWrite(transKernel, 0) << "{  " << last_datablk_idx << ",  " << last_datablk_idx << ",  0},"
                                        << std::endl;
            ...
            for (size_t i = 0; i < num_cycles_minus_1; i++) {
                clKernWrite(transKernel, 0) << "{  " << cycle_stat[i * 2] << ",  " << cycle_stat[i * 2 + 1] << "},"
                                            << std::endl;
            }
            clKernWrite(transKernel, 0) << "{  " << cycle_stat[num_cycles_minus_1 * 2] << ",  "
            ...
            switch (params.fft_inputLayout) {
                case CLFFT_COMPLEX_INTERLEAVED:
                    clKernWrite(transKernel, 0) << "void swap(global " << dtComplex << "* inputA, " << tmpBuffType
                                                << " " << dtComplex << "* Ls, " << tmpBuffType << " " << dtComplex
                                                << " * Ld, size_t is, size_t id, size_t pos, size_t end_indx, size_t work_id";
                    break;
                case CLFFT_COMPLEX_PLANAR:
                    clKernWrite(transKernel, 0) << "void swap(global " << dtPlanar << "* inputA_R, global " << dtPlanar
                ...
                case CLFFT_HERMITIAN_PLANAR:
                    return CLFFT_TRANSPOSED_NOTIMPLEMENTED;
                case CLFFT_REAL:
                    clKernWrite(transKernel, 0) << "void swap(global " << dtPlanar << "* inputA, " << tmpBuffType << " "
            ...
            switch (params.fft_inputLayout) {
                ...
                case CLFFT_COMPLEX_INTERLEAVED:
                    ...
                    if (params.fft_hasPreCallback) {
                        ...
                        clKernWrite(transKernel, 9) << "Ls[j] = " << params.fft_preCallback.funcname
                        ...
                        clKernWrite(transKernel, 9) << "Ld[j] = " << params.fft_preCallback.funcname
                        ...
                        clKernWrite(transKernel, 6) << "if (pos == 0){" << std::endl;
                        clKernWrite(transKernel, 9) << "Ls[j] = inputA[is *" << smaller_dim << " + "
                                                    << num_elements_loaded << " * work_id + j];" << std::endl;
                        clKernWrite(transKernel, 9) << "Ld[j] = inputA[id *" << smaller_dim << " + "
                                                    << num_elements_loaded << " * work_id + j];" << std::endl;
                        clKernWrite(transKernel, 6) << "}" << std::endl;
                    ...
                    if (params.fft_hasPostCallback) {
                        clKernWrite(transKernel, 6) << params.fft_postCallback.funcname << "(inputA, (iOffset + id*"
                        ...
                        clKernWrite(transKernel, 6) << "inputA[id*" << smaller_dim << " + " << num_elements_loaded
                ...
                case CLFFT_COMPLEX_PLANAR:
                    if (params.fft_hasPreCallback) {
                        ...
                        clKernWrite(transKernel, 6) << "if (pos == 0){" << std::endl;
                        clKernWrite(transKernel, 9) << "Ls[j].x = inputA_R[is*" << smaller_dim << " + "
                                                    << num_elements_loaded << " * work_id + j];" << std::endl;
                        clKernWrite(transKernel, 9) << "Ls[j].y = inputA_I[is*" << smaller_dim << " + "
                                                    << num_elements_loaded << " * work_id + j];" << std::endl;
                        clKernWrite(transKernel, 9) << "Ld[j].x = inputA_R[id*" << smaller_dim << " + "
                                                    << num_elements_loaded << " * work_id + j];" << std::endl;
                        clKernWrite(transKernel, 9) << "Ld[j].y = inputA_I[id*" << smaller_dim << " + "
                                                    << num_elements_loaded << " * work_id + j];" << std::endl;
                        clKernWrite(transKernel, 6) << "}" << std::endl;
                    ...
                    if (params.fft_hasPostCallback) {
                        ...
                        clKernWrite(transKernel, 6) << "inputA_R[id*" << smaller_dim << " + " << num_elements_loaded
                                                    << " * work_id + j] = Ls[j].x;" << std::endl;
                        clKernWrite(transKernel, 6) << "inputA_I[id*" << smaller_dim << " + " << num_elements_loaded
            ...
            clKernWrite(transKernel, 3) << "const size_t numGroupsY_1 = " << cycle_map[0] * num_grps_pro_row << " ;"
                                        << std::endl;
            for (size_t i = 2; i < params.fft_DataDim - 1; i++) {
                clKernWrite(transKernel, 3) << "const size_t numGroupsY_" << i << " = numGroupsY_" << i - 1 << " * "
            ...
            switch (params.fft_inputLayout) {
                ...
                case CLFFT_REAL:
                    ...
                    clKernWrite(transKernel, 3) << "__local " << dtInput << " tmp_tot_mem[" << (num_elements_loaded * 2)
                                                << "];" << std::endl;
                    clKernWrite(transKernel, 3) << tmpBuffType << " " << dtInput << " *te = tmp_tot_mem;" << std::endl;

                    clKernWrite(transKernel, 3) << tmpBuffType << " " << dtInput << " *to = (tmp_tot_mem + "
                ...
                case CLFFT_COMPLEX_PLANAR:
                    ...
                    clKernWrite(transKernel, 3) << "__local " << dtComplex << " tmp_tot_mem["
                                                << (num_elements_loaded * 2) << "];" << std::endl;
                    clKernWrite(transKernel, 3) << tmpBuffType << " " << dtComplex << " *te = tmp_tot_mem;"
                                                << std::endl;

                    clKernWrite(transKernel, 3) << tmpBuffType << " " << dtComplex << " *to = (tmp_tot_mem + "
                                                << num_elements_loaded << ");" << std::endl;
            ...
            clKernWrite(transKernel, 3) << "size_t start = cycle_stat[g_index / " << num_grps_pro_row << "][0];"
                                        << std::endl;
            clKernWrite(transKernel, 3) << "size_t end = cycle_stat[g_index / " << num_grps_pro_row << "][1];"
                                        << std::endl;
            ...
            clKernWrite(transKernel, 6) << "end_indx = " << smaller_dim - num_elements_loaded * (num_grps_pro_row - 1)
                                        << ";" << std::endl;
            clKernWrite(transKernel, 3) << "}" << std::endl;
            ...
            switch (params.fft_inputLayout) {
                ...
                case CLFFT_COMPLEX_PLANAR:
                    clKernWrite(transKernel, 6)
                            << "swap(inputA_R, inputA_I, tmp_swap_ptr[swap_inx], tmp_swap_ptr[1 - swap_inx], swap_table[loop][0], swap_table[loop][1], swap_table[loop][2], end_indx, work_id";
                    break;
            }
            if (params.fft_hasPostCallback) {
...
//this function accepts any ratio in theory. But in practice we restrict it to 1:2, 1:3, 1:5 and 1:10 ration
    clfftStatus
    genSwapKernelGeneral(const FFTGeneratedTransposeNonSquareAction::Signature &params, std::string &strKernel,
                         std::string &KernelFuncName, const size_t &lwSize, const size_t reShapeFactor) {
        if (params.fft_placeness == CLFFT_OUTOFPLACE)
            return CLFFT_TRANSPOSED_NOTIMPLEMENTED;
    ...
    */
        ...
        while (LDS_per_WG >
               1024)//avoiding using too much lds memory. the biggest LDS memory we will allocate would be 1024*sizeof(float2/double2)*2
        {
            if (LDS_per_WG % 2 == 0) {
                LDS_per_WG /= 2;
                continue;
    ...
    */
        ...
        bool twiddleSwapKernelOut = twiddleSwapKernel && (params.nonSquareKernelOrder == TRANSPOSE_AND_SWAP ||
        ...
        clKernWrite(transKernel, 0) << "__constant size_t swap_table[" << permutationTable.size() + 2 << "][1] = {"
                                    << std::endl;
        clKernWrite(transKernel, 0) << "{0}," << std::endl;
        clKernWrite(transKernel, 0) << "{" << smaller_dim * dim_ratio - 1 << "},"
                                    << std::endl;// add the first and last row to the swap table. needed for twiddling
        for (std::vector<std::vector<size_t> >::iterator itor = permutationTable.begin();
        ...
        std::string smaller_dim_str = static_cast<const std::ostringstream &>(std::ostringstream()
                << smaller_dim).str();
        ...
        for (size_t bothDir = 0; bothDir < 2; bothDir++) {
            ...
            genTransposePrototypeLeadingDimensionBatched(params, local_work_size_swap, dtPlanar, dtComplex, funcNameTW,
                                                         transKernel, dtInput, dtOutput);

            clKernWrite(transKernel, 3) << "//each wg handles 1/" << WG_per_line << " row of " << LDS_per_WG
                                        << " in memory" << std::endl;
            clKernWrite(transKernel, 3) << "const size_t num_wg_per_batch = "
            ...
            switch (params.fft_inputLayout) {
                case CLFFT_REAL:
                case CLFFT_COMPLEX_INTERLEAVED:
                    clKernWrite(transKernel, 3) << "inputA += batch_offset*" << smaller_dim * bigger_dim << ";"
                                                << std::endl;
                    break;
                case CLFFT_HERMITIAN_INTERLEAVED:
                case CLFFT_HERMITIAN_PLANAR:
                    return CLFFT_TRANSPOSED_NOTIMPLEMENTED;
                case CLFFT_COMPLEX_PLANAR: {
                    clKernWrite(transKernel, 3) << "inputA_R += batch_offset*" << smaller_dim * bigger_dim << ";"
                                                << std::endl;
                    clKernWrite(transKernel, 3) << "inputA_I += batch_offset*" << smaller_dim * bigger_dim << ";"
                                                << std::endl;
                    break;
                }
                default:
            ...
            clKernWrite(transKernel, 3) << "group_id -= batch_offset*" << (permutationTable.size() + 2) * WG_per_line
                                        << ";" << std::endl;
            ...
            if (WG_per_line == 1)
                ...
                clKernWrite(transKernel, 3) << "size_t prev = swap_table[group_id/" << WG_per_line << "][0];"
                                            << std::endl;
            ...
            switch (params.fft_inputLayout) {
                case CLFFT_REAL:
                case CLFFT_COMPLEX_INTERLEAVED: {
                    clKernWrite(transKernel, 3) << "__local " << dtInput << " prevValue[" << LDS_per_WG << "];"
                                                << std::endl;//lds within each wg should be able to store a row block (smaller_dim) of element
                    clKernWrite(transKernel, 3) << "__local " << dtInput << " nextValue[" << LDS_per_WG << "];"
                                                << std::endl;
                    break;
                }
                case CLFFT_COMPLEX_PLANAR: {
                    clKernWrite(transKernel, 3) << "__local " << dtComplex << " prevValue[" << LDS_per_WG << "];"
                                                << std::endl;//lds within each wg should be able to store a row block (smaller_dim) of element
                    clKernWrite(transKernel, 3) << "__local " << dtComplex << " nextValue[" << LDS_per_WG << "];"
                                                << std::endl;
                    break;
                }
                case CLFFT_HERMITIAN_INTERLEAVED:
            ...
            if (params.fft_N[0] > params.fft_N[1])//decides whether we have a tall or wide rectangle
            {
                if (WG_per_line == 1) {
                    ...
                    clKernWrite(transKernel, 3) << "size_t group_offset = (prev/" << dim_ratio << ")*" << smaller_dim
                                                << "*" << dim_ratio
                    ...
                    clKernWrite(transKernel, 3) << "size_t group_offset = (prev/" << dim_ratio << ")*" << smaller_dim
                                                << "*" << dim_ratio
                                                << " + (prev%" << dim_ratio << ")*" << smaller_dim << " + (group_id % "
                ...
                if (WG_per_line == 1)//might look like: size_t group_offset = prev*729;
                    ...
                    clKernWrite(transKernel, 3) << "size_t group_offset = (prev*" << smaller_dim << ") + (group_id % "
                                                << WG_per_line << ")*" << LDS_per_WG << ";" << std::endl;
            }
                        ...
                        if (i + 256 < LDS_per_WG) {
                            if (params.fft_hasPreCallback) {
                                ...
                                if (twiddleSwapKernelIn) {
                                        ...
                                        params.fft_N[1])//decides whether we have a tall or wide rectangle
                                    {
                                        ...
                                        clKernWrite(transKernel, 6) << "p = (group_offset+idx+" << i << ")/"
                                                                    << bigger_dim << ";" << std::endl;
                                        clKernWrite(transKernel, 6) << "q = (group_offset+idx+" << i << ")%"
                                                                    << bigger_dim << ";" << std::endl;
                                    } else {
                                    ...
                                    if (fwd) {
                                        //forward
                                        clKernWrite(transKernel, 3) << "prevValue[idx+" << i
                                                                    << "].x = inputA[group_offset+idx+" << i
                                                                    << "].x * twiddle_factor.x - inputA[group_offset+idx+"
                                                                    << i << "].y * twiddle_factor.y;" << std::endl;
                                        clKernWrite(transKernel, 3) << "prevValue[idx+" << i
                                                                    << "].y = inputA[group_offset+idx+" << i
                                                                    << "].x * twiddle_factor.y + inputA[group_offset+idx+"
                                        ...
                                        clKernWrite(transKernel, 3) << "prevValue[idx+" << i
                                                                    << "].y = inputA[group_offset+idx+" << i
                                                                    << "].y * twiddle_factor.x - inputA[group_offset+idx+"
                                                                    << i << "].x * twiddle_factor.y;" << std::endl;
                                    }
                                } else
                                    clKernWrite(transKernel, 3) << "prevValue[idx+" << i
                                                                << "] = inputA[group_offset+idx+" << i << "];"
                                                                << std::endl;
                            }
                        } else {
                            ...
                            if (params.fft_hasPreCallback) {
                                ...
                                if (twiddleSwapKernelIn) {
                                    ...
                                    clKernWrite(transKernel, 6) << "prevValue[idx+" << i
                                                                << "] = inputA[group_offset+idx+" << i << "];"
                                                                << std::endl;
                            }
                            clKernWrite(transKernel, 3) << "}" << std::endl;
                                ...
                                if (twiddleSwapKernelIn) {
                                    ...
                                    if (fwd) {
                                        ...
                                        clKernWrite(transKernel, 3) << "prevValue[idx+" << i
                                                                    ...
                                                                    << "] * twiddle_factor.x - inputA_I[group_offset+idx+"
                                                                    << i << "] * twiddle_factor.y;" << std::endl;
                                        clKernWrite(transKernel, 3) << "prevValue[idx+" << i
                                                                    << "].y = inputA_R[group_offset+idx+" << i
                                                                    << "] * twiddle_factor.y + inputA_I[group_offset+idx+"
                                        ...
                                        clKernWrite(transKernel, 3) << "prevValue[idx+" << i
                                                                    ...
                                                                    << "] * twiddle_factor.x - inputA_R[group_offset+idx+"
                                                                    << i << "] * twiddle_factor.y;" << std::endl;
                                    }
                                } else {
                                    clKernWrite(transKernel, 3) << "prevValue[idx+" << i
            ...
            if (params.fft_N[0] > params.fft_N[1])//decides whether we have a tall or wide rectangle
            {
                clKernWrite(transKernel, 6) << "next = (prev*" << smaller_dim << ")%" << smaller_dim * dim_ratio - 1
                                            << ";" << std::endl;
                ...
                if (WG_per_line == 1) {
                    clKernWrite(transKernel, 6) << "group_offset = (next/" << dim_ratio << ")*" << smaller_dim << "*"
                                                << dim_ratio
                                                << " + (next%" << dim_ratio << ")*" << smaller_dim << ";"
                                                << std::endl; //might look like: group_offset = (next/3)*729*3 + (next%3)*729;
                } else {
                    //if smaller_dim is 2187 > 1024 this should look like size_t group_offset = (next/3)*2187*3 + (next%3)*2187 + (group_id % 3)*729;
                    clKernWrite(transKernel, 6) << "group_offset = (next/" << dim_ratio << ")*" << smaller_dim << "*"
                                                << dim_ratio
                                                << " + (next%" << dim_ratio << ")*" << smaller_dim << " + (group_id % "
                                                << WG_per_line << ")*" << LDS_per_WG << ";" << std::endl;
                }
            } else {
                clKernWrite(transKernel, 6) << "next = (prev*" << dim_ratio << ")%" << smaller_dim * dim_ratio - 1
                                            << ";" << std::endl;
                //takes care the last row
                clKernWrite(transKernel, 6) << "if (prev == " << smaller_dim * dim_ratio - 1 << ")" << std::endl;
                clKernWrite(transKernel, 9) << "next = " << smaller_dim * dim_ratio - 1 << ";" << std::endl;
                if (WG_per_line == 1) //might look like: size_t group_offset = prev*729;
                    ...
                    clKernWrite(transKernel, 6) << "group_offset = (next*" << smaller_dim << ") + (group_id % "
                                                << WG_per_line << ")*" << LDS_per_WG << ";" << std::endl;
            }
            ...
            switch (params.fft_inputLayout) {
                ...
                case CLFFT_COMPLEX_INTERLEAVED: {
                    for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
                        if (i + 256 < LDS_per_WG)
                            if (params.fft_hasPreCallback) {
                                clKernWrite(transKernel, 6) << "nextValue[idx+" << i << "] = "
                                                            << params.fft_preCallback.funcname
                                                            << "(inputA-batch_offset*" << smaller_dim * bigger_dim
                                                            << ", batch_offset*" << smaller_dim * bigger_dim
                                                            << "+group_offset+idx+" << i << ", pre_userdata);"
                                ...
                                if (twiddleSwapKernelIn) {
                                    ...
                                    if (fwd) {
                                        //forward
                                        clKernWrite(transKernel, 3) << "nextValue[idx+" << i
                                                                    << "].x = inputA[group_offset+idx+" << i
                                                                    << "].x * twiddle_factor.x - inputA[group_offset+idx+"
                                                                    << i << "].y * twiddle_factor.y;" << std::endl;
                                        clKernWrite(transKernel, 3) << "nextValue[idx+" << i
                            ...
                            if (params.fft_hasPreCallback) {
                                ...
                                if (twiddleSwapKernelIn) {
                                    ...
                                    clKernWrite(transKernel, 9) << "nextValue[idx+" << i
                                                                << "] = inputA[group_offset+idx+" << i << "];"
                                                                << std::endl;
                            }
                            clKernWrite(transKernel, 6) << "}" << std::endl;
                        ...
                        for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
                            if (i + 256 < LDS_per_WG) {
                                if (params.fft_N[0] > params.fft_N[1])//decides whether we have a tall or wide rectangle
                                {
                                    ...
                                    clKernWrite(transKernel, 6) << "p = (group_offset+idx+" << i << ")/" << smaller_dim
                                                                << ";" << std::endl;
                                    clKernWrite(transKernel, 6) << "q = (group_offset+idx+" << i << ")%" << smaller_dim
                                                                << ";" << std::endl;
                                } else {
                                    //input is tall; output is wide
                                    clKernWrite(transKernel, 6) << "p = (group_offset+idx+" << i << ")/" << bigger_dim
                                                                << ";" << std::endl;
                                    clKernWrite(transKernel, 6) << "q = (group_offset+idx+" << i << ")%" << bigger_dim
                                                                << ";" << std::endl;
                                }
                                clKernWrite(transKernel, 6) << "twiddle_factor = TW3step(p*q);" << std::endl;
                                if (fwd) {
                        ...
                        for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
                            ...
                            if (params.fft_hasPostCallback) {
                                if (i + 256 < LDS_per_WG) {
                                    ...
                                    // need to handle boundary
                                    clKernWrite(transKernel, 6) << "if(idx+" << i << "<" << LDS_per_WG << "){"
                                                                << std::endl;
                                    clKernWrite(transKernel, 9) << params.fft_postCallback.funcname
                                                                << "(inputA - batch_offset*" << smaller_dim * bigger_dim
                                                                << ", batch_offset*" << smaller_dim * bigger_dim
                                                                << "+group_offset+idx+" << i
                                                                << ", post_userdata, prevValue[idx+" << i
                    ...
                    if (twiddleSwapKernelOut) {
                        ...
                        for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
                            if (i + 256 < LDS_per_WG) {
                                ...
                                if (fwd) {
                                    //forward
                                    clKernWrite(transKernel, 6) << "inputA_R[group_offset+idx+" << i
                                                                << "] = prevValue[idx+" << i
                                                                << "].x * twiddle_factor.x - prevValue[idx+" << i
                                                                << "].y * twiddle_factor.y;" << std::endl;
                                    clKernWrite(transKernel, 6) << "inputA_I[group_offset+idx+" << i
                                                                << "] = prevValue[idx+" << i
                                                                << "].x * twiddle_factor.y + prevValue[idx+" << i
                                                                << "].y * twiddle_factor.x;" << std::endl;
                                } else {
                        ...
                        for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
                            ...
                            if (params.fft_hasPostCallback) {
                                if (i + 256 < LDS_per_WG) {
                                    ...
                                    clKernWrite(transKernel, 6) << params.fft_postCallback.funcname
                                                                << "(inputA_R - batch_offset*"
                                                                << smaller_dim * bigger_dim
                                                                << ", inputA_I - batch_offset*"
                                                                << smaller_dim * bigger_dim << ", batch_offset*"
                                                                << smaller_dim * bigger_dim
                                                                << "+group_offset+idx+" << i
                                                                << ", post_userdata, prevValue[idx+" << i
            ...
            switch (params.fft_inputLayout) {
                ...
                case CLFFT_COMPLEX_PLANAR: {
                    for (size_t i = 0; i < LDS_per_WG; i = i + 256) {
                        if (i + 256 < LDS_per_WG)
                            clKernWrite(transKernel, 6) << "prevValue[idx+" << i << "] = nextValue[idx+" << i << "];"
                                                        << std::endl;
                        else {
                            // need to handle boundary
                            clKernWrite(transKernel, 6) << "if(idx+" << i << "<" << LDS_per_WG << "){" << std::endl;
                            clKernWrite(transKernel, 9) << "prevValue[idx + " << i << "] = nextValue[idx + " << i
                                                        << "]; " << std::endl;
                            clKernWrite(transKernel, 6) << "}" << std::endl;
                        }
                    }
            ...
            if (WG_per_line == 1)
                ...
                clKernWrite(transKernel, 3) << "}while(next!=swap_table[group_id/" << WG_per_line << "][0]);"
...
*/
    clfftStatus
    genTransposeKernelBatched(const FFTGeneratedTransposeSquareAction::Signature &params, std::string &strKernel,
        ...
        bool twiddleTransposeKernel = params.fft_3StepTwiddle && (params.transposeMiniBatchSize ==
                                                                  1);//when transposeMiniBatchSize == 1 it is guaranteed to be a sqaure matrix transpose
        ...
        for (size_t bothDir = 0; bothDir < 2; bothDir++) {
            ...
            if (params.fft_placeness == CLFFT_OUTOFPLACE) {
                switch (params.fft_outputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED:
                        clKernWrite(transKernel, 3) << "outputA += oOffset;"
                                                    << std::endl;  // Set A ptr to the start of each slice

                        break;
                    case CLFFT_COMPLEX_PLANAR:

                        clKernWrite(transKernel, 3) << "outputA_R += oOffset;"
                                                    << std::endl;  // Set A ptr to the start of each slice
                        clKernWrite(transKernel, 3) << "outputA_I += oOffset;"
                                                    << std::endl;  // Set A ptr to the start of each slice
                        break;
                    case CLFFT_HERMITIAN_INTERLEAVED:
                    case CLFFT_HERMITIAN_PLANAR:
                ...
                switch (params.fft_inputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED:
                        if (params.fft_hasPreCallback)
                            clKernWrite(transKernel, 3) << "global " << dtInput << " *outputA = inputA + iOffset;"
                                                        << std::endl;
                        else
                    ...
                    case CLFFT_COMPLEX_PLANAR:
                        if (params.fft_hasPreCallback) {
                            clKernWrite(transKernel, 3) << "global " << dtInput << " *outputA_R = inputA_R + iOffset;"
                                                        << std::endl;
                            clKernWrite(transKernel, 3) << "global " << dtInput << " *outputA_I = inputA_I + iOffset;"
                                                        << std::endl;
                        } else {
                            clKernWrite(transKernel, 3) << "global " << dtInput << " *outputA_R = inputA_R;"
                                                        << std::endl;
                            clKernWrite(transKernel, 3) << "global " << dtInput << " *outputA_I = inputA_I;"
                                                        << std::endl;
                        }
                        break;
                    case CLFFT_HERMITIAN_INTERLEAVED:
            ...
            if (mult_of_16)
                clKernWrite(transKernel, 3) << "float row = (" << -2.0f * params.fft_N[0] / 16 / reShapeFactor - 1
                                            << "+sqrt((" << 4.0f * params.fft_N[0] / 16 / reShapeFactor *
                                                            (params.fft_N[0] / 16 / reShapeFactor + 1)
                                            << "-8.0f*g_index- 7)))/ (-2.0f);" << std::endl;
            else
                clKernWrite(transKernel, 3) << "float row = ("
                                            << -2.0f * (params.fft_N[0] / (16 * reShapeFactor) + 1) - 1 << "+sqrt(("
            ...
            if (mult_of_16)
                clKernWrite(transKernel, 3) << "const long t_gx_p = g_index - "
                                            << (params.fft_N[0] / 16 / reShapeFactor) << "*t_gy + t_gy*(t_gy + 1) / 2;"
                                            << std::endl;
            else
                clKernWrite(transKernel, 3) << "const long t_gx_p = g_index - "
                                            << (params.fft_N[0] / (16 * reShapeFactor) + 1)
            ...
            clKernWrite(transKernel, 3) << "const size_t lidy = (d_lidy * 16 + d_lidx) /" << (16 * reShapeFactor) << ";"
                                        << std::endl;
            clKernWrite(transKernel, 3) << "const size_t lidx = (d_lidy * 16 + d_lidx) %" << (16 * reShapeFactor) << ";"
                                        << std::endl;

            clKernWrite(transKernel, 3) << "" << std::endl;

            clKernWrite(transKernel, 3) << "const size_t idx = lidx + t_gx_p*" << 16 * reShapeFactor << ";"
                                        << std::endl;
            clKernWrite(transKernel, 3) << "const size_t idy = lidy + t_gy_p*" << 16 * reShapeFactor << ";"
                                        << std::endl;

            clKernWrite(transKernel, 3) << "" << std::endl;

            clKernWrite(transKernel, 3) << "const size_t starting_index_yx = t_gy_p*" << 16 * reShapeFactor
                                        << " + t_gx_p*" << 16 * reShapeFactor * params.fft_N[0] << ";" << std::endl;

            clKernWrite(transKernel, 3) << "" << std::endl;

            clKernWrite(transKernel, 3) << "__local " << dtComplex << " xy_s["
                                        << 16 * reShapeFactor * 16 * reShapeFactor << "];" << std::endl;
            clKernWrite(transKernel, 3) << "__local " << dtComplex << " yx_s["
                                        << 16 * reShapeFactor * 16 * reShapeFactor << "];" << std::endl;
            ...
            if (mult_of_16) {
                ...
                clKernWrite(transKernel, 3) << "for (size_t loop = 0; loop<" << reShapeFactor * reShapeFactor
                                            << "; ++loop){" << std::endl;
                clKernWrite(transKernel, 6) << "index = lidy*" << 16 * reShapeFactor << " + lidx + loop*256;"
                                            << std::endl;
                ...
                switch (params.fft_inputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED: {
                        if (params.fft_hasPreCallback) {
                            if (params.fft_preCallback.localMemSize > 0) {
                                clKernWrite(transKernel, 6) << "tmpm = " << params.fft_preCallback.funcname
                                                            << "(inputA, iOffset + (idy + loop * " << 16 / reShapeFactor
                                                            << ")*" << params.fft_N[0]
                                                            << " + idx, pre_userdata, localmem);" << std::endl;
                                clKernWrite(transKernel, 6) << "tmpt = " << params.fft_preCallback.funcname
                                                            << "(inputA, iOffset + (lidy + loop * "
                                ...
                                clKernWrite(transKernel, 6) << "tmpt = " << params.fft_preCallback.funcname
                                                            << "(inputA, iOffset + (lidy + loop * "
                                                            << 16 / reShapeFactor << ")*" << params.fft_N[0]
                                                            << " + lidx + starting_index_yx, pre_userdata);"
                                                            << std::endl;
                            }
                        } else {
                            clKernWrite(transKernel, 6) << "tmpm = inputA[(idy + loop *" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0] << " + idx];" << std::endl;
                            clKernWrite(transKernel, 6) << "tmpt = inputA[(lidy + loop *" << 16 / reShapeFactor << ")*"
                    ...
                    case CLFFT_COMPLEX_PLANAR:
                        ...
                        if (params.fft_hasPreCallback) {
                            ...
                            clKernWrite(transKernel, 6) << "tmpm.x = inputA_R[(idy + loop *" << 16 / reShapeFactor
                                                        << ")*" << params.fft_N[0] << " + idx];" << std::endl;
                            clKernWrite(transKernel, 6) << "tmpm.y = inputA_I[(idy + loop *" << 16 / reShapeFactor
                                                        << ")*" << params.fft_N[0] << " + idx];" << std::endl;

                            clKernWrite(transKernel, 6) << "tmpt.x = inputA_R[(lidy + loop *" << 16 / reShapeFactor
                                                        << ")*" << params.fft_N[0] << " + lidx + starting_index_yx];"
                                                        << std::endl;
                            clKernWrite(transKernel, 6) << "tmpt.y = inputA_I[(lidy + loop *" << 16 / reShapeFactor
                                                        << ")*" << params.fft_N[0] << " + lidx + starting_index_yx];"
                                                        << std::endl;
                        }
                        break;
                ...
                clKernWrite(transKernel, 3) << "for (size_t loop = 0; loop<" << reShapeFactor * reShapeFactor
                                            << "; ++loop){" << std::endl;
                clKernWrite(transKernel, 6) << "index = lidx*" << 16 * reShapeFactor << " + lidy + "
                    ...
                    case CLFFT_COMPLEX_INTERLEAVED:
                        if (params.fft_hasPostCallback) {
                            if (params.transposeMiniBatchSize <
                                2)//which means the matrix was not broken down into sub square matrics
                                clKernWrite(transKernel, 6) << params.fft_postCallback.funcname
                                                            << "(outputA, ((idy + loop*" << 16 / reShapeFactor << ")*"
                                                            << params.fft_N[0] << " + idx), post_userdata, yx_s[index]";
                            else {
                                ...
                                clKernWrite(transKernel, 6) << params.fft_postCallback.funcname << "(outputA-"
                                                            << blockOffset << "*((get_group_id(0)/numGroupsY_1)%"
                                                            << params.transposeMiniBatchSize << "), ((idy + loop*"
                                                            << 16 / reShapeFactor << ")*"
                                                            << params.fft_N[0] << " + idx + " << blockOffset
                            ...
                            clKernWrite(transKernel, 6) << "outputA[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0] << " + idx] = yx_s[index];" << std::endl;
                            clKernWrite(transKernel, 6) << "outputA[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0]
                    ...
                    case CLFFT_COMPLEX_PLANAR:
                        if (params.fft_hasPostCallback) {
                            if (params.transposeMiniBatchSize <
                                2)//which means the matrix was not broken down into sub square matrics
                                clKernWrite(transKernel, 6) << params.fft_postCallback.funcname
                                                            << "(outputA_R, outputA_I, ((idy + loop*"
                                                            << 16 / reShapeFactor << ")*" << params.fft_N[0]
                                                            << " + idx), post_userdata, yx_s[index].x, yx_s[index].y";
                            else {
                                size_t blockOffset = params.fft_inStride[2];
                                clKernWrite(transKernel, 6) << params.fft_postCallback.funcname << "(outputA_R - "
                                                            << blockOffset << "*((get_group_id(0)/numGroupsY_1)%"
                                                            << params.transposeMiniBatchSize <<
                                                            "), outputA_I -" << blockOffset
                                                            << "*((get_group_id(0)/numGroupsY_1)%"
                                                            << params.transposeMiniBatchSize <<
                                                            "), ((idy + loop*" << 16 / reShapeFactor << ")*"
                                                            << params.fft_N[0] << " + idx +" << blockOffset
                                                            << "*((get_group_id(0)/numGroupsY_1)%"
                                                            << params.transposeMiniBatchSize <<
                            ...
                            if (params.transposeMiniBatchSize <
                                2)//which means the matrix was not broken down into sub square matrics
                                clKernWrite(transKernel, 6) << params.fft_postCallback.funcname
                                                            << "(outputA_R, outputA_I, ((lidy + loop*"
                                                            << 16 / reShapeFactor << ")*" << params.fft_N[0]
                                                            << " + lidx+ starting_index_yx), post_userdata, xy_s[index].x, xy_s[index].y";
                            else {
                                size_t blockOffset = params.fft_inStride[2];
                                clKernWrite(transKernel, 6) << params.fft_postCallback.funcname << "(outputA_R - "
                                                            << blockOffset << "*((get_group_id(0)/numGroupsY_1)%"
                            ...
                            clKernWrite(transKernel, 6) << "outputA_R[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0] << " + idx] = yx_s[index].x;" << std::endl;
                            clKernWrite(transKernel, 6) << "outputA_I[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0] << " + idx] = yx_s[index].y;" << std::endl;

                            clKernWrite(transKernel, 6) << "outputA_R[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0]
                                                        << " + lidx+ starting_index_yx] = xy_s[index].x;" << std::endl;
                            clKernWrite(transKernel, 6) << "outputA_I[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0]
                ...
                clKernWrite(transKernel, 3) << "size_t index;" << std::endl;
                clKernWrite(transKernel, 3) << "if (" << params.fft_N[0] << " - (t_gx_p + 1) *" << 16 * reShapeFactor
                                            << ">0){" << std::endl;
                clKernWrite(transKernel, 6) << "for (size_t loop = 0; loop<" << reShapeFactor * reShapeFactor
                                            << "; ++loop){" << std::endl;
                clKernWrite(transKernel, 9) << "index = lidy*" << 16 * reShapeFactor << " + lidx + loop*256;"
                                            << std::endl;
                ...
                switch (params.fft_inputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED:
                        if (params.fft_hasPreCallback) {
                            if (params.fft_preCallback.localMemSize > 0) {
                                clKernWrite(transKernel, 9) << "tmpm = " << params.fft_preCallback.funcname
                                                            << "(inputA, iOffset + (idy + loop*" << 16 / reShapeFactor
                                                            << ")*" << params.fft_N[0]
                                                            << " + idx, pre_userdata, localmem);" << std::endl;
                                clKernWrite(transKernel, 9) << "tmpt = " << params.fft_preCallback.funcname
                            ...
                            clKernWrite(transKernel, 9) << "tmpm = inputA[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0] << " + idx];" << std::endl;
                            clKernWrite(transKernel, 9) << "tmpt = inputA[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0] << " + lidx + starting_index_yx];"
                                                        << std::endl;
                        }
                        break;
                    case CLFFT_COMPLEX_PLANAR:
                        ...
                        if (params.fft_hasPreCallback) {
                            ...
                            clKernWrite(transKernel, 9) << "tmpm.x = inputA_R[(idy + loop*" << 16 / reShapeFactor
                                                        << ")*" << params.fft_N[0] << " + idx];" << std::endl;
                            clKernWrite(transKernel, 9) << "tmpm.y = inputA_I[(idy + loop*" << 16 / reShapeFactor
                                                        << ")*" << params.fft_N[0] << " + idx];" << std::endl;

                            clKernWrite(transKernel, 9) << "tmpt.x = inputA_R[(lidy + loop*" << 16 / reShapeFactor
                                                        << ")*" << params.fft_N[0] << " + lidx + starting_index_yx];"
                                                        << std::endl;
                            clKernWrite(transKernel, 9) << "tmpt.y = inputA_I[(lidy + loop*" << 16 / reShapeFactor
                                                        << ")*" << params.fft_N[0] << " + lidx + starting_index_yx];"
                                                        << std::endl;
                        }
                        break;
                ...
                switch (params.fft_inputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED:
                        ...
                        if (params.fft_hasPreCallback) {
                            if (params.fft_preCallback.localMemSize > 0) {
                                clKernWrite(transKernel, 12) << "tmpm = " << params.fft_preCallback.funcname
                                                             << "(inputA, iOffset + (idy + loop*" << 16 / reShapeFactor
                                                             << ")*" << params.fft_N[0]
                                                             << " + idx, pre_userdata, localmem);" << std::endl;
                                clKernWrite(transKernel, 9) << "if ((t_gy_p *" << 16 * reShapeFactor << " + lidx)<"
                                                            << params.fft_N[0] << " && (t_gx_p * " << 16 * reShapeFactor
                                                            << " + lidy + loop*" << 16 / reShapeFactor << ")<"
                                                            << params.fft_N[0] << ") " << std::endl;
                                clKernWrite(transKernel, 12) << "tmpt = " << params.fft_preCallback.funcname
                                                             << "(inputA, iOffset + (lidy + loop*" << 16 / reShapeFactor
                                                             << ")*" << params.fft_N[0]
                                                             << " + lidx + starting_index_yx, pre_userdata, localmem);"
                                                             << std::endl;
                            ...
                            clKernWrite(transKernel, 12) << "tmpm = inputA[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                         << params.fft_N[0] << " + idx];" << std::endl;
                            clKernWrite(transKernel, 9) << "if ((t_gy_p *" << 16 * reShapeFactor << " + lidx)<"
                                                        << params.fft_N[0] << " && (t_gx_p * " << 16 * reShapeFactor
                                                        << " + lidy + loop*" << 16 / reShapeFactor << ")<"
                                                        << params.fft_N[0] << ") " << std::endl;
                            clKernWrite(transKernel, 12) << "tmpt = inputA[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                         << params.fft_N[0] << " + lidx + starting_index_yx];"
                                                         << std::endl;
                        }
                        break;
                    case CLFFT_COMPLEX_PLANAR:
                        ...
                        if (params.fft_hasPreCallback) {
                            ...
                            clKernWrite(transKernel, 12) << "tmpm.x = inputA_R[(idy + loop*" << 16 / reShapeFactor
                                                         << ")*" << params.fft_N[0] << " + idx];" << std::endl;
                            clKernWrite(transKernel, 12) << "tmpm.y = inputA_I[(idy + loop*" << 16 / reShapeFactor
                                                         << ")*" << params.fft_N[0] << " + idx]; }" << std::endl;
                            clKernWrite(transKernel, 9) << "if ((t_gy_p *" << 16 * reShapeFactor << " + lidx)<"
                                                        << params.fft_N[0] << " && (t_gx_p * " << 16 * reShapeFactor
                                                        << " + lidy + loop*" << 16 / reShapeFactor << ")<"
                                                        << params.fft_N[0] << ") {" << std::endl;
                            clKernWrite(transKernel, 12) << "tmpt.x = inputA_R[(lidy + loop*" << 16 / reShapeFactor
                                                         << ")*" << params.fft_N[0] << " + lidx + starting_index_yx];"
                                                         << std::endl;
                            clKernWrite(transKernel, 12) << "tmpt.y = inputA_I[(lidy + loop*" << 16 / reShapeFactor
                                                         << ")*" << params.fft_N[0] << " + lidx + starting_index_yx]; }"
                                                         << std::endl;
                        }
                        break;
                ...
                clKernWrite(transKernel, 6) << "for (size_t loop = 0; loop<" << reShapeFactor * reShapeFactor
                                            << "; ++loop){" << std::endl;
                clKernWrite(transKernel, 9) << "index = lidx*" << 16 * reShapeFactor << " + lidy + "
                                            << 16 / reShapeFactor << "*loop ;" << std::endl;
                ...
                switch (params.fft_outputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED:
                        if (params.fft_hasPostCallback) {
                            ...
                            clKernWrite(transKernel, 9) << "outputA[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0] << " + idx] = yx_s[index];" << std::endl;
                            clKernWrite(transKernel, 9) << "outputA[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0]
                                                        << " + lidx + starting_index_yx] = xy_s[index]; " << std::endl;
                        }
                        break;
                    case CLFFT_COMPLEX_PLANAR:
                        if (params.fft_hasPostCallback) {
                            if (params.transposeMiniBatchSize <
                                ...
                                clKernWrite(transKernel, 9) << params.fft_postCallback.funcname
                                                            << "(outputA_R-iOffset, outputA_I-iOffset, iOffset+((idy + loop*"
                                                            << 16 / reShapeFactor << ")*" << params.fft_N[0]
                                                            << " + idx), post_userdata, yx_s[index].x, yx_s[index].y";
                            if (params.fft_postCallback.localMemSize > 0) {
                                clKernWrite(transKernel, 0) << ", localmem";
                            ...
                            if (params.transposeMiniBatchSize <
                                ...
                                clKernWrite(transKernel, 9) << params.fft_postCallback.funcname
                                                            << "(outputA_R-iOffset, outputA_I-iOffset, iOffset+((lidy + loop*"
                                                            << 16 / reShapeFactor << ")*" << params.fft_N[0]
                                                            << " + lidx + starting_index_yx), post_userdata, xy_s[index].x, xy_s[index].y";
                            if (params.fft_postCallback.localMemSize > 0) {
                                clKernWrite(transKernel, 0) << ", localmem";
                            ...
                            clKernWrite(transKernel, 9) << "outputA_R[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0] << " + idx] = yx_s[index].x;" << std::endl;
                            clKernWrite(transKernel, 9) << "outputA_I[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0] << " + idx] = yx_s[index].y;" << std::endl;
                            clKernWrite(transKernel, 9) << "outputA_R[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0]
                                                        << " + lidx + starting_index_yx] = xy_s[index].x; "
                                                        << std::endl;
                            clKernWrite(transKernel, 9) << "outputA_I[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                        << params.fft_N[0]
                                                        << " + lidx + starting_index_yx] = xy_s[index].y; "
                                                        << std::endl;
                        }
                        break;
                    case CLFFT_HERMITIAN_INTERLEAVED:
                ...
                switch (params.fft_outputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED:
                        ...
                        if (params.fft_hasPostCallback) {
                            if (params.transposeMiniBatchSize <
                                2)//which means the matrix was not broken down into sub square matrics
                                clKernWrite(transKernel, 12) << params.fft_postCallback.funcname
                                                             << "(outputA, ((idy + loop*" << 16 / reShapeFactor << ")*"
                                                             << params.fft_N[0]
                                                             << " + idx), post_userdata, yx_s[index]";
                            else
                            ...
                            clKernWrite(transKernel, 12) << "outputA[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                         << params.fft_N[0] << " + idx] = yx_s[index]; " << std::endl;
                            clKernWrite(transKernel, 9) << "if ((t_gy_p * " << 16 * reShapeFactor << " + lidx)<"
                                                        << params.fft_N[0] << " && (t_gx_p * " << 16 * reShapeFactor
                                                        << " + lidy + loop*" << 16 / reShapeFactor << ")<"
                                                        << params.fft_N[0] << ")" << std::endl;
                            clKernWrite(transKernel, 12) << "outputA[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                         << params.fft_N[0]
                                                         << " + lidx + starting_index_yx] = xy_s[index];" << std::endl;
                        }
                        break;
                    case CLFFT_COMPLEX_PLANAR:
                        ...
                        if (params.fft_hasPostCallback) {
                            ...
                            clKernWrite(transKernel, 12) << "outputA_R[(idy + loop*" << 16 / reShapeFactor << ")*"
                                                         << params.fft_N[0] << " + idx] = yx_s[index].x; " << std::endl;
                            clKernWrite(transKernel, 12) << "outputA_I[(idy + loop*" << 16 / reShapeFactor << ")*"
                            ...
                            clKernWrite(transKernel, 9) << "if ((t_gy_p * " << 16 * reShapeFactor << " + lidx)<"
                                                        ...
                                                        << " + lidy + loop*" << 16 / reShapeFactor << ")<"
                                                        << params.fft_N[0] << ") {" << std::endl;
                            clKernWrite(transKernel, 12) << "outputA_R[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                         << params.fft_N[0]
                                                         << " + lidx + starting_index_yx] = xy_s[index].x;"
                                                         << std::endl;
                            clKernWrite(transKernel, 12) << "outputA_I[(lidy + loop*" << 16 / reShapeFactor << ")*"
                                                         << params.fft_N[0]
                                                         << " + lidx + starting_index_yx] = xy_s[index].y; }"
                                                         << std::endl;
                        }
...
[M0 M2 M2]
*/
    clfftStatus genTransposeKernelLeadingDimensionBatched(const FFTGeneratedTransposeNonSquareAction::Signature &params,
                                                          std::string &strKernel, const size_t &lwSize,
        ...
        for (size_t bothDir = 0; bothDir < 2; bothDir++) {
            ...
            // Generate kernel API
            genTransposePrototypeLeadingDimensionBatched(params, lwSize, dtPlanar, dtComplex, funcName, transKernel,
                                                         dtInput, dtOutput);

            if (mult_of_16)//number of WG per sub square block
                clKernWrite(transKernel, 3) << "const size_t  numGroups_square_matrix_Y_1 = "
                                            << (smaller_dim / 16 / reShapeFactor) *
                                               (smaller_dim / 16 / reShapeFactor + 1) / 2 << ";" << std::endl;
            else
                clKernWrite(transKernel, 3) << "const size_t  numGroups_square_matrix_Y_1 = "
                                            << (smaller_dim / (16 * reShapeFactor) + 1) *
                                               (smaller_dim / (16 * reShapeFactor) + 1 + 1) / 2 << ";" << std::endl;

            clKernWrite(transKernel, 3) << "const size_t  numGroupsY_1 =  numGroups_square_matrix_Y_1 * " << dim_ratio
                                        << ";" << std::endl;
            ...
            clKernWrite(transKernel, 3) << "square_matrix_index = (g_index / numGroups_square_matrix_Y_1) ;"
                                        << std::endl;
            clKernWrite(transKernel, 3) << "g_index = g_index % numGroups_square_matrix_Y_1" << ";" << std::endl;
            clKernWrite(transKernel, 3) << std::endl;

            if (smaller_dim == params.fft_N[1]) {
                clKernWrite(transKernel, 3) << "square_matrix_offset = square_matrix_index * " << smaller_dim << ";"
                                            << std::endl;
            } else {
                clKernWrite(transKernel, 3) << "square_matrix_offset = square_matrix_index *"
            ...
            // for a triangular indexing
            if (mult_of_16)
                clKernWrite(transKernel, 3) << "float row = (" << -2.0f * smaller_dim / 16 / reShapeFactor - 1
                                            << "+sqrt((" << 4.0f * smaller_dim / 16 / reShapeFactor *
                                                            (smaller_dim / 16 / reShapeFactor + 1)
                                            << "-8.0f*g_index- 7)))/ (-2.0f);" << std::endl;
            else
                clKernWrite(transKernel, 3) << "float row = (" << -2.0f * (smaller_dim / (16 * reShapeFactor) + 1) - 1
                                            << "+sqrt((" << 4.0f * (smaller_dim / (16 * reShapeFactor) + 1) *
            ...
            if (mult_of_16)
                clKernWrite(transKernel, 3) << "const long t_gx_p = g_index - " << (smaller_dim / 16 / reShapeFactor)
                                            << "*t_gy + t_gy*(t_gy + 1) / 2;" << std::endl;
            else
                clKernWrite(transKernel, 3) << "const long t_gx_p = g_index - "
                                            << (smaller_dim / (16 * reShapeFactor) + 1)
            ...
            switch (params.fft_inputLayout) {
                case CLFFT_REAL:
                case CLFFT_COMPLEX_INTERLEAVED:
                    clKernWrite(transKernel, 3) << "__local " << dtInput << " xy_s["
                                                << 16 * reShapeFactor * 16 * reShapeFactor << "];" << std::endl;
                    clKernWrite(transKernel, 3) << "__local " << dtInput << " yx_s["
                                                << 16 * reShapeFactor * 16 * reShapeFactor << "];" << std::endl;
            ...
            if (mult_of_16) {
                ...
                switch (params.fft_outputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED:
                        if (params.fft_hasPostCallback) {
                            clKernWrite(transKernel, 6) << params.fft_postCallback.funcname << "(outputA, ((idy + loop*"
                                                        << 16 / reShapeFactor << ")*" << params.fft_N[0]
                                                        << " + idx), post_userdata, yx_s[index]";
                            if (params.fft_postCallback.localMemSize > 0) {
                                clKernWrite(transKernel, 0) << ", localmem";
                ...
                clKernWrite(transKernel, 3) << "size_t index;" << std::endl;
                clKernWrite(transKernel, 3) << "if (" << smaller_dim << " - (t_gx_p + 1) *" << 16 * reShapeFactor
                                            << ">0){" << std::endl;
                clKernWrite(transKernel, 6) << "for (size_t loop = 0; loop<" << reShapeFactor * reShapeFactor
                                            << "; ++loop){" << std::endl;
                clKernWrite(transKernel, 9) << "index = lidy*" << 16 * reShapeFactor << " + lidx + loop*256;"
                ...
                switch (params.fft_inputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED:
                    case CLFFT_REAL:
                        clKernWrite(transKernel, 9) << "if ((idy + loop*" << 16 / reShapeFactor << ")<" << smaller_dim
                ...
                switch (params.fft_outputLayout) {
                    case CLFFT_COMPLEX_INTERLEAVED:
                        if (params.fft_hasPostCallback) {
                            clKernWrite(transKernel, 9) << params.fft_postCallback.funcname << "(outputA, ((idy + loop*"
                                                        << 16 / reShapeFactor << ")*" << params.fft_N[0]
                                                        << " + idx), post_userdata, yx_s[index]";
                            if (params.fft_postCallback.localMemSize > 0) {
                                clKernWrite(transKernel, 0) << ", localmem";
